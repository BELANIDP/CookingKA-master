...
/**
 * File        : android.presenter.fragments.favorites.FavoritesDoneNessLevelFragment
 * Brief       : Assisted picking DoneNess values for saving as favorites
 * Author      : VYASM
 * Created On  : 23/10/2024
 * Details     : User can select done ness ex light, medium, rare, etc from the string tumbler
 */
class FavoritesDoneNessLevelFragment : AbstractStringTumblerFragment(),
    ...
    private fun updateCtaRightButton() {
        tumblerViewHolderHelper?.providePrimaryButton()?.text =
            if (cookingViewModel.recipeExecutionViewModel.isRunning) getString(R.string.text_button_update) else getString(
