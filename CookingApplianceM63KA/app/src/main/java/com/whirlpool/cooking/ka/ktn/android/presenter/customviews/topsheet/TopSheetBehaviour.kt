...
class TopSheetBehavior<V : View?> : CoordinatorLayout.Behavior<V?> {
    ...
    override fun onSaveInstanceState(parent: CoordinatorLayout, child: V & Any): Parcelable {
    ...
    override fun onLayoutChild(
        ...
        if (checkFitsSystemWindows(parent) && !checkFitsSystemWindows(child)) {
            setFitsSystemWindows(child, true)
        }
    ...
    private fun checkFitsSystemWindows(parent: View): Boolean {
        val insets: WindowInsetsCompat? = ViewCompat.getRootWindowInsets(parent)
        return insets != null && insets.isVisible(WindowInsetsCompat.Type.systemBars())
    }
    @Suppress("SameParameterValue")
    private fun setFitsSystemWindows(child: View, fitsSystemWindows: Boolean) {
    ...
    override fun onStopNestedScroll(
        ...
        } else if (mHideable && getYVelocity()?.let { shouldHide(child, it) } == true) {
            top = -child.height
     ...
     */
    private fun setSkipCollapsed(skipCollapsed: Boolean) {
    ...
    fun setAllowUserDragging(isDrag: Boolean) {
        this.isDrag = isDrag
    }
}
