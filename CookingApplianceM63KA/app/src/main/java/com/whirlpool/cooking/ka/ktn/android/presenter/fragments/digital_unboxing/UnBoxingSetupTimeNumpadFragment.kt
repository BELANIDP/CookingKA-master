 ...
 */
class UnBoxingSetupTimeNumpadFragment : AbstractDateTimeFragment(), HMIKnobInteractionListener,
    ...
    private fun updateDateTimeValues(dateTimeValue: String) {
        if (dateTimeValue != AppConstants.EMPTY_STRING) {
            ...
            if (!TextUtils.isEmpty(hour) && !TextUtils.isEmpty(minute)) {
                ...
                HMILogHelper.Logd("Unboxing", "dateTimeText = $dateTimeText")
                numberPadViewHolder?.provideDateTimeValueTextView()?.setValue(dateTimeText)
            }
    ...
    private fun checkForPreviousTimeValues() {
        if (arguments != null && arguments?.getString(BundleKeys.BUNDLE_PROVISIONING_TIME) != null) {
            if (arguments?.getBoolean(BundleKeys.BUNDLE_IS_FROM_TOOLS_MENU) == true || arguments?.getString(
                ...
                if (settingsViewModel?.is12HourTimeFormat()?.getValue() == true) {
                    // 12-hour format
            ...
            HMILogHelper.Logd("Unboxing", "Save timeString ----$timeString")
            if (timeString != null) {
                updateDateTimeValues(timeString)
            }
        }
    ...
    private fun onNavigateToVerticalTumbler() {
        ...
        HMILogHelper.Logd("Unboxing", "Navigate vertical tumbler = $timeValue")
        NavigationUtils.navigateSafely(
            this,
            R.id.action_unboxingSetupTimeFragment_to_time12HrsTumblerFragment,
            bundle,
            null
        )
    }
    ...
    private fun onNextButtonClick(): View.OnClickListener {
        return View.OnClickListener {
            if (keyboardViewModel?.isValidEntry == true) {
                HMILogHelper.Logd("Unboxing", "Time valid entry = true")
                val navigationId = R.id.action_unboxingSetupTimeFragment_to_dateTumblerFragment
                if (updateTimeAndNavigateOut(this, dateTimeValue, navigationId)) {
                    ...
                    arguments?.clear()
                    HMILogHelper.Logv("Unboxing", "Set Time successfully")
                } else {
                    HMILogHelper.Loge("Unboxing", "Not able to set the Time")
                }
            } else {
                HMILogHelper.Logd("Unboxing", "time valid entry = false")
                showInvalidMessage(
    ...
    private fun initializeView() {
        ...
        numberPadViewHolder?.provideDateTimeValueTextView()?.apply {
            setMaxCharacterLength(AppConstants.MAX_CHAR_LENGTH)
            setClearEntryOnLimitReached(true)
            setTextViewType(KeyboardTextFormatUtils.TEXT_TYPE_CLOCK)
    ...
    private fun getErrorDetails(): String {
        ...
        HMILogHelper.Logd("Unboxing", "Keyboard validEntry = $validEntry")
