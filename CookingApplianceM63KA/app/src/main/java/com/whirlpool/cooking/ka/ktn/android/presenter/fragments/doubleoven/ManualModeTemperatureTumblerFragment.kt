...
/**
 * File       : com.whirlpool.cooking.ka.ktn.android.presenter.fragments.double oven.ManualModeTemperatureTumblerFragment.
...
class ManualModeTemperatureTumblerFragment : AbstractNumericTumblerFragment(),
    ...
    override fun onKnobRotateEvent(knobId: Int, knobDirection: String) {
        ...
        //TODO Paused for now due to UI glitch
//        else if (knobId == AppConstants.LEFT_KNOB_ID) {
//            animateSelectedKnob(knobDirection)
//        }
    }

    @Suppress("unused")
    private fun animateSelectedKnob(knobDirection: String) {
        when (knobDirection) {
            KnobDirection.CLOCK_WISE_DIRECTION -> {
                CommonAnimationUtils.animateViewX(
                    getBinding()?.selectionLine!!,
                    getBinding()?.btnGhost?.x!! ,
                    getBinding()?.btnPrimary?.x!! - 100,
                    R.integer.ms_300.toLong()
                )
            }
            KnobDirection.COUNTER_CLOCK_WISE_DIRECTION -> {
                CommonAnimationUtils.animateViewX(
                    getBinding()?.selectionLine!!,
                    getBinding()?.btnPrimary?.x!! ,
                    getBinding()?.btnGhost?.x!! - 30,
                    R.integer.ms_300.toLong()
                )
            }
        }
