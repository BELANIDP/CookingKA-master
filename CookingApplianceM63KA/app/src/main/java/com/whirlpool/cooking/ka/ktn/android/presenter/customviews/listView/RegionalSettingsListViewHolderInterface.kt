...
import android.presenter.customviews.radiobutton.UnderLineButton
...
import com.whirlpool.cooking.ka.databinding.ItemRegionalSettingsListTileBinding
import com.whirlpool.hmi.uicomponents.widgets.list.ListViewInterface
import core.utils.AppConstants

/**
 * File        : android.presenter.customviews.listView.RegionalSettingsListViewHolderInterface
 ...
 */
open class RegionalSettingsListViewHolderInterface(
    private val dataModelList: ArrayList<ListTileData>,
    private val listItemClickListener: ListItemClickListener
) : ListViewInterface {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val binding = ItemRegionalSettingsListTileBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    protected inner class ListItemViewHolder internal constructor(private var itemListTileLayoutBinding: ItemRegionalSettingsListTileBinding) :
        ...
        fun bind(item: ListTileData,position: Int) {
            ...
            itemListTileLayoutBinding.settingsItemUnderlineToggleSwitch.visibility = item.underLineButton.visibility

            if(itemListTileLayoutBinding.settingsItemUnderlineToggleSwitch.visibility == View.VISIBLE) {
                itemListTileLayoutBinding.settingsItemUnderlineToggleSwitch.setChecked(item.underLineButton.isChecked)
                itemListTileLayoutBinding.settingsItemUnderlineToggleSwitch.getLeftTextView().text = item.underLineButton.leftButtonText
                itemListTileLayoutBinding.settingsItemUnderlineToggleSwitch.getRightTextView().text = item.underLineButton.rightButtonText
                itemListTileLayoutBinding.settingsItemUnderlineToggleSwitch.setUnderLineItemClickListener(object :UnderLineButton.ListItemClickListener{
                    override fun onToggleSwitchClick(isChecked: Boolean) {
                        listItemClickListener.onToggleSwitchClick(position,isChecked)
                    }
                })
