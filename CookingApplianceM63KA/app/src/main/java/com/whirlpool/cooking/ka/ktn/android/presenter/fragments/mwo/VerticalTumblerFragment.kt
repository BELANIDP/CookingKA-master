...
/**
 * File       : android.presenter.fragments.mwo
...
class VerticalTumblerFragment : AbstractVerticalTumblerFragment(),
    ...
    override fun updateCTARightText() {
        if (cookingViewModel?.isOfTypeMicrowaveOven == true && cookingViewModel?.recipeExecutionViewModel?.cookTimerState?.value == Timer.State.IDLE) {
            ...
            if (requiredRecipeOptions?.contains(RecipeOptions.COOK_TIME) == true && requiredRecipeOptions.contains(
                ...
                tumblerViewHolderHelper?.providePrimaryButton()?.setTextButtonText(getString(R.string.text_button_start))
                return
            }
        }
        return super.updateCTARightText()
    }
    ...
    private fun navigateToCookTimeNumPadScreen() {
        var selectedMinute = tumblerViewHolderHelper?.provideVerticalTumblerLeft()?.selectedValue
        var selectedSecond = tumblerViewHolderHelper?.provideVerticalTumblerCenter()?.selectedValue
        selectedMinute = String.format(
            Locale.getDefault(),
            AppConstants.DEFAULT_DATE_VALUE_FORMAT,
            selectedMinute?.toInt()
        )
        selectedSecond = String.format(
            Locale.getDefault(),
            AppConstants.DEFAULT_DATE_VALUE_FORMAT,
            selectedSecond?.toInt()
        )
        HMILogHelper.Logd(" cook time numpad-->$selectedMinute$selectedSecond")

        val bundle = Bundle()
        bundle.putString(BundleKeys.BUNDLE_PROVISIONING_TIME, "$selectedMinute$selectedSecond")
