...
/**
 * File        : android.presenter.fragments.doubleoven.SingleStatusFragment
 ...
 */
class SingleSabbathLowerStatusFragment : AbstractStatusFragment(), HMIExpansionUtils.HMIErrorCodesListener {
    ...
    override fun onCreateView(
        ...
        statusViewHelper = SingleSabbathLowerStatusViewHelper()
    ...
    override fun onStart() {
        ...
        if(MeatProbeUtils.isMeatProbeConnected(statusViewHelper.getLowerViewModel()))
    ...
    override fun onHMIFaultId(cookingViewModel: CookingViewModel, faultId: Int?) {
        HMILogHelper.Logd(tag, "SABBATH Fault SingleLowerStatus, cavityName ${cookingViewModel.cavityName.value}, faultId $faultId")
        if (faultId != 0) {
            ...
            if(CookingAppUtils.isFaultAorC(faultDetails)){
                HMILogHelper.Logd(tag, "SABBATH Fault SingleLowerStatus, cavityName ${cookingViewModel.cavityName.value}, faultDetails ${faultDetails?.getFaultCode()} is Type A 0r C so cancelling for both cavity")
    ...
    override fun onHMICommunicationFaultCode(
        ...
        HMILogHelper.Logd(tag, "SABBATH Fault SingleLowerStatus, cavityName ${cookingViewModel.cavityName.value}, communicationFaultCode $communicationFaultCode")
    }
    /***************************** KNOB Related *********************************/
