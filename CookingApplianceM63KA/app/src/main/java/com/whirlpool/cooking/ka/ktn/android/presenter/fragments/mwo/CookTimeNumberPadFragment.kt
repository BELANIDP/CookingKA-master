...
/**
 * File        : android.presenter.fragments.mwo.CookTimeNumberPadFragment.
 * Brief       : Instance of clock time numberPad for MWO
 * Author      : GHARDNS <br>
...
class CookTimeNumberPadFragment : AbstractCookTimeNumberPadFragment(),
    ...
    override fun onRightButtonClick() {
        ...
        if (cookTimeText != AppConstants.DEFAULT_COOK_TIME_MICROWAVE) {
            if (validateCookTime()) {
                ...
                if (powerAndTimeOptions && cookTimeNumberPadViewHolderHelper?.getLeftPowerTextButton()?.visibility == View.VISIBLE && cookingViewModel?.recipeExecutionViewModel?.cookTimerState?.value == Timer.State.IDLE) {
    ...
    override fun updateRightButtonText(): String {
        if (cookingViewModel?.recipeExecutionViewModel?.cookTimerState?.value == Timer.State.IDLE) {
            val requiredRecipeOptions =
                cookingViewModel?.recipeExecutionViewModel?.requiredOptions?.value
