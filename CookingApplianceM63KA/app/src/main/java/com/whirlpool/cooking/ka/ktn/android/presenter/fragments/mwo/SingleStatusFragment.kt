...
/**
 * File        : android.presenter.fragments.mwo.SingleStatusFragment
            ...
            if(!CookingAppUtils.isUserInstructionRequired(cookingVM)) {
                ...
                if(cookingVM.recipeExecutionViewModel.recipeExecutionState.value == RecipeExecutionState.PAUSED ||
                    ...
                    when (cookingVM.recipeExecutionViewModel?.isMagnetronUsed) {
                        true -> {
                            HMILogHelper.Logd("Door","MWO: Showing MagnetronUsed based popup")
                            ...
                            HMILogHelper.Logd("Door","MWO: Showing Non - MagnetronUsed based popup")
                            NavigationUtils.getVisibleFragment()?.let {
    ...
    override fun provideKnobRotationItems(): ArrayList<KnobItem> {
        ...
        if(upperWidgetHelper?.tvResumeCooking()?.isVisible == true) {
            ...
            if(upperWidgetHelper?.isCookTimeNotAllowed() == false)
                arrayList.add(KnobItem(upperWidgetHelper.tvSetCookTime(), false, arrayList.size,provideViewHolderHelper().getUpperViewModel()))
            arrayList.add(KnobItem(upperWidgetHelper?.tvOvenStateAction(), false, arrayList.size,provideViewHolderHelper().getUpperViewModel()))
            arrayList.add(KnobItem(upperWidgetHelper?.getCavityMoreMenu(), false, arrayList.size,provideViewHolderHelper().getUpperViewModel()))
            return arrayList
        }
    }
