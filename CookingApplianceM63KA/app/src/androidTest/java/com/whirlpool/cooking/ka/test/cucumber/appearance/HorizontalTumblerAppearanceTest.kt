...
//        UiTestingUtils.isViewVisible(R.id.button_2)
    ...
    fun horizontalScrollLeft() {
        UiTestingUtils.scrollToRecyclerViewIndex(R.id.tumblerNumericBased, 0)
    }

    fun horizontalScrollRight() {
        UiTestingUtils.scrollToRecyclerViewIndex(R.id.tumblerNumericBased, 10)
    }
    ...
    fun checkTumblerHeight(): Boolean {
        ...
        onView(withId(R.id.tumblerNumericBased)).check { view, _ ->
            if (view is BaseTumbler) {
                ...
                val layout =
                    itemView?.findViewById<ConstraintLayout>(com.whirlpool.hmi.cooking.R.id.base_tumbler_item_text_view)
    ...
    fun checkIsContainerPaddingProper() {
        ...
        assert(isValid) { "The container padding is as expected" }
    }
    ...
    fun checkIsContainerPaddingProperNotProper() {
        ...
        assert(!isValid) { "he container padding is not  expected" }
    }
    ...
    fun isTextViewVisibleInItem(
        viewId: Int, childId: Int, isHidden: Boolean, position: Int
    ): Boolean {
    ...
    fun validateTextViewSize(
        viewId: Int, childId: Int, position: Int, expectedTextSize: Int
    ): Boolean {
    ...
    fun validateTextViewColor(
        viewId: Int, childId: Int, position: Int, expectedColor: Int
    ): Boolean {
    ...
    fun validateContainerPadding(
        viewId: Int,
        position: Int,
        expectedPaddingLeft: Int,
        expectedPaddingRight: Int,
        expectedPaddingTop: Int,
        expectedPaddingBottom: Int
    ): Boolean {
        var isPaddingAsExpected = false
        ...
        onView(withId(viewId)).check { view, _ ->
            if (view is BaseTumbler) {
                ...
                layout?.let {
                    ...
                    // Check if all padding values match the expected padding
                    isPaddingAsExpected =
                        paddingTop == expectedPaddingTop && paddingBottom == expectedPaddingBottom && paddingLeft == expectedPaddingLeft && paddingRight == expectedPaddingRight
                }
            }
        }
        return isPaddingAsExpected
